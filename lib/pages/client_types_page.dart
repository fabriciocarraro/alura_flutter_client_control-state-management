...
class _ClientTypesPageState extends State<ClientTypesPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Consumer<Types>(
          builder: (BuildContext context, Types typesList, Widget? widget) {
        return ListView.builder(
          itemCount: typesList.types.length,
          itemBuilder: (context, index) {
            return Dismissible(
              key: UniqueKey(),
              background: Container(color: Colors.red),
              child: ListTile(
                leading: Icon(typesList.types[index].icon),
                title: Text(typesList.types[index].name),
